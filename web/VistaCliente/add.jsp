        ...
        <div class="form-container">
            ...
            <form action="${pageContext.request.contextPath}/srvCliente" method="post" onsubmit="return validarFormulario();">
                ...
                <input type="text" name="nombre" id="nombre"
                       value="${not empty mensajeError ? clienteForm.nombre : ''}"
                       maxlength="50"
                       required
                ...
                <input type="text" name="apellido" id="apellido"
                       value="${not empty mensajeError ? clienteForm.apellido : ''}"
                       maxlength="50"
                       required
                       pattern="[A-Za-zÁÉÍÓÚáéíóúÑñ ]+"
                       title="Solo letras y espacios. Máximo 50 caracteres."
                ...
                <select name="idTipoDocumento" id="idTipoDocumento" required>
                    ...
                    <c:forEach var="tipo" items="${listaTiposDocumento}">
                        <option value="${tipo.idTipoDocumento}"
                                ${not empty mensajeError and clienteForm.idTipoDocumento == tipo.idTipoDocumento ? 'selected="selected"' : ''}>${tipo.nombre}</option>
                    </c:forEach>
                </select>

                <label for="numeroDocumento">Número de documento:</label>
                <input type="text" name="numeroDocumento" id="numeroDocumento"
                       value="${not empty mensajeError ? clienteForm.numeroDocumento : ''}"
                       pattern="[0-9]{8,15}"
                       maxlength="15"
                       required
                ...
                <input type="text" name="telefono" id="telefono"
                       value="${not empty mensajeError ? clienteForm.telefono : ''}"
                       pattern="[0-9]{6,15}"
                       maxlength="15"
                       required
                ...
                <input type="text" name="direccion" id="direccion"
                       value="${not empty mensajeError ? clienteForm.direccion : ''}"
                       minlength="5"
                       maxlength="80"
                       required
                       oninput="this.value = this.value.toUpperCase();">
                ...
                <input type="email" name="email" id="email"
                       value="${not empty mensajeError ? clienteForm.email : ''}"
                       maxlength="80"
                       required
                ...
                <div class="password-row">
                    <input type="password" name="clave" id="clave"
                           value="${not empty mensajeError ? clienteForm.claveVisible : ''}"
                           minlength="4" maxlength="30" required>
                    <button type="button" class="toggle-password" onclick="togglePassword('clave', 'iconoClaveAddCliente');">
                        <span id="iconoClaveAddCliente">👁️</span>
                    </button>
                </div>

                <label for="estado">Estado:</label>
                <select name="estado" id="estado" required>
                    <option value="1" ${not empty mensajeError and clienteForm.estado == 1 ? 'selected="selected"' : ''}>Activo</option>
                    <option value="0" ${not empty mensajeError and clienteForm.estado == 0 ? 'selected="selected"' : ''}>Inactivo</option>
                </select>
