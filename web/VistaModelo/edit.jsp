...
<div class="form-container">
    ...
    <form action="${pageContext.request.contextPath}/srvModelo" method="post" onsubmit="return validarFormulario();">
        <input type="hidden" name="accion" value="actualizar">
        <input type="hidden" name="id" value="${modelo.idModelo}">

        <label for="idMarca">Marca:</label>
        <select name="idMarca" id="idMarca" required>
            <option value="">Seleccione una marca</option>
            <c:forEach var="marca" items="${listaMarcas}">
                <option value="${marca.idMarca}" ${marca.idMarca == modelo.idMarca ? 'selected' : ''}>
                    ${marca.nombre}
                </option>
            </c:forEach>
        </select>
        ...
        <select name="estado" id="estado" required>
            <option value="1" ${modelo.estado == 1 ? 'selected' : ''}>Activo</option>
            <option value="0" ${modelo.estado == 0 ? 'selected' : ''}>Inactivo</option>
        </select>
